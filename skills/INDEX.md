...
description: Entry point for the social content generation system. Two apps (ManifestLock, JournalLock), two personas (Sophie, Sanya), three accounts. Agent reads this first, then traverses relevant nodes.
---
...
|---------|---------|-----|-----------|
| @sophie.unplugs | Sophie | ManifestLock | 1 |
| @emillywilks | Sophie | ManifestLock | 1 |
| @sanyahealing | Sanya | JournalLock | 1 |

**Total: 3 posts/day.** Sophie's two accounts must have different content each day.

## The Products

Two apps, same core mechanic (lock apps until daily practice), different practices:

- [[manifest-lock]] — locks apps until users read manifestations aloud. Promoted by Sophie on @sophie.unplugs and @emillywilks. Morning routine energy.
- [[journal-lock]] — locks apps until users journal for the day. Promoted by Sanya on @sanyahealing. Late-night overthinking energy.
...
- [[sophie]] — dark hair, band tee, outdoor park. Dry, grounded, skeptic-turned-believer. Runs 2 ManifestLock accounts (@sophie.unplugs, @emillywilks).
- [[sanya]] — brunette, red sweatshirt, cozy room. Warm, vulnerable, overthinker. Runs 1 JournalLock account (@sanyahealing).
