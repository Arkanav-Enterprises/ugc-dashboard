...
def load_context_for_account(account: str) -> str:
    """Build the Anthropic system prompt from relevant skill graph nodes."""
...
def run_account(account: str, category_override: str | None = None,
    ...
    # 3. Check dedup (Sophie's second account)
    dedup_hooks = []
    if account == "emillywilks":
        sophie_output = load_today_output("sophie.unplugs")
        if sophie_output:
            dedup_hooks = [sophie_output["content"]["pov_text"]]
            print(f"  Dedup: avoiding hook from @sophie.unplugs")
